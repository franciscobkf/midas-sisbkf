<?php
/**
 * SIS MIDAS v3.22 - API Principal
 * Sistema de Gestão Financeira - BKF Group
 * * FIX v3.22:
 * - Correção crítica nos CRUDs de Categorias e Fontes (erro de campo vazio).
 * - Proteção contra "Undefined array key" em todos os endpoints POST/PUT.
 */
...
try {
    ...
    http_response_code(500); echo json_encode(['success' => false, 'error' => 'Conexão: ' . $e->getMessage()]); exit;
}
...
try {
    switch ($action) {
        ...
        case 'dashboard':
            ...
            $partes = explode('-', $mes_ano); $mes_nome = $meses_pt[$partes[1]] . ' ' . $partes[0];
            ...
            $stmt->execute([$cotacao, $cotacao, $cotacao, $mes_ano]); $rec = $stmt->fetch();
            $resumo_mes['receitas_recebidas']=$rec['recebidas']; $resumo_mes['receitas_pendentes']=$rec['pendentes']; $resumo_mes['total_receitas']=$rec['total'];
            ...
            $stmt->execute([$cotacao, $cotacao, $cotacao, $mes_ano]); $desp = $stmt->fetch();
            $resumo_mes['despesas_pagas']=$desp['pagas']; $resumo_mes['despesas_pendentes']=$desp['pendentes']; $resumo_mes['total_despesas']=$desp['total'];
        ...
        case 'auxiliares':
            ...
            $meses=[]; $pt=['01'=>'Janeiro','02'=>'Fevereiro','03'=>'Março','04'=>'Abril','05'=>'Maio','06'=>'Junho','07'=>'Julho','08'=>'Agosto','09'=>'Setembro','10'=>'Outubro','11'=>'Novembro','12'=>'Dezembro'];
            for($i=-12;$i<=3;$i++){ $d=new DateTime(); $d->modify("$i months"); $meses[]=['valor'=>$d->format('Y-m'),'label'=>$pt[$d->format('m')].' '.$d->format('Y')]; }
        ...
        case 'categorias': 
            if($_SERVER['REQUEST_METHOD']==='GET'){
                $s=$pdo->query("SELECT * FROM categorias WHERE ativo=1 ORDER BY nome"); respond($s->fetchAll());
            } elseif($_SERVER['REQUEST_METHOD']==='POST'){
                $d=getJsonInput(); 
                // FIX: Adicionado ?? '' para evitar erro se campo não vier do JS
                $pdo->prepare("INSERT INTO categorias (nome, descricao, cor, icone, ativo) VALUES (?,?,?,?,1)")
                    ->execute([$d['nome']??'', $d['descricao']??'', $d['cor']??'#6b7280', $d['icone']??'📁']); 
                respond(['id'=>$pdo->lastInsertId(), 'message'=>'Criado']);
            } elseif($_SERVER['REQUEST_METHOD']==='DELETE'){
                $pdo->prepare("UPDATE categorias SET ativo=0 WHERE id=?")->execute([$_GET['id']??0]); respond(['message'=>'Removido']);
            } 
            break;

        // ==================== FONTES DE RECEITA (CORRIGIDO) ====================
        case 'fontes_receita': 
            if($_SERVER['REQUEST_METHOD']==='GET'){
                $s=$pdo->query("SELECT * FROM fontes_receita WHERE ativo=1 ORDER BY nome"); respond($s->fetchAll());
            } elseif($_SERVER['REQUEST_METHOD']==='POST'){
                $d=getJsonInput(); 
                $pdo->prepare("INSERT INTO fontes_receita (nome, descricao, ativo) VALUES (?,?,1)")
                    ->execute([$d['nome']??'', $d['descricao']??'']); 
                respond(['id'=>$pdo->lastInsertId(), 'message'=>'Criado']);
            } elseif($_SERVER['REQUEST_METHOD']==='DELETE'){
                $pdo->prepare("UPDATE fontes_receita SET ativo=0 WHERE id=?")->execute([$_GET['id']??0]); respond(['message'=>'Removido']);
            } 
            break;
        ...
        case 'clientes_cadastro':
            ...
            if($_SERVER['REQUEST_METHOD']==='GET' && !$id) {
                ...
                $stmt->execute([$cotacao, $cotacao]); respond($stmt->fetchAll());
            } elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
                $d = getJsonInput();
                ...
                respond(['id'=>$pdo->lastInsertId(), 'message'=>'Cliente criado']);
                 ...
                 $pdo->commit(); respond(['message'=>'Cliente desativado']);
            }
            break;
        ...
        case 'cliente':
            $id = $_GET['id'] ?? 0;
            if ($_SERVER['REQUEST_METHOD'] === 'GET') {
                $stmt = $pdo->prepare("SELECT * FROM clientes_cadastro WHERE id=?"); $stmt->execute([$id]); $cliente = $stmt->fetch(PDO::FETCH_ASSOC);
                if(!$cliente){http_response_code(404); respond(['error'=>'404'],false);}
                $stmt=$pdo->prepare("SELECT * FROM receitas WHERE cliente_id=? AND status!='encerrado' ORDER BY created_at DESC"); $stmt->execute([$id]); $cliente['receitas']=$stmt->fetchAll(PDO::FETCH_ASSOC);
                $stmt=$pdo->prepare("SELECT * FROM clientes_anexos WHERE cliente_id=? ORDER BY created_at DESC"); $stmt->execute([$id]); $cliente['anexos']=$stmt->fetchAll();
                $stmt=$pdo->prepare("SELECT * FROM lancamentos WHERE cliente_id=? ORDER BY data_vencimento DESC LIMIT 20"); $stmt->execute([$id]); $cliente['lancamentos']=$stmt->fetchAll();
                respond($cliente);
            }
            break;
        ...
        case 'receitas':
            if ($_SERVER['REQUEST_METHOD'] === 'GET') {
                ...
                $d=getJsonInput(); $stmt=$pdo->prepare("INSERT INTO receitas (cliente_id, fonte_receita_id, descricao, valor, moeda, tipo, recorrencia, dia_vencimento, data_inicio, data_fim, nivel_esforco, equipe, score_roi, status, observacoes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                ...
                respond(['id'=>$pdo->lastInsertId(), 'message'=>'Receita criada']);
            } elseif ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
                 $id = $_GET['id'] ?? 0;
                 $pdo->beginTransaction();
                 ...
                 $pdo->commit(); respond(['message'=>'Receita encerrada']);
            }
            break;
        ...
        case 'receita':
            $id=$_GET['id']??0;
            if ($_SERVER['REQUEST_METHOD'] === 'GET') {
                $stmt=$pdo->prepare("SELECT r.*, c.razao_social as cliente_nome FROM receitas r LEFT JOIN clientes_cadastro c ON r.cliente_id=c.id WHERE r.id=?"); $stmt->execute([$id]);
                $receita=$stmt->fetch(PDO::FETCH_ASSOC); if(!$receita) respond(['error'=>'404'],false); respond($receita);
            } elseif ($_SERVER['REQUEST_METHOD'] === 'PUT') {
                $d=getJsonInput(); $stmt=$pdo->prepare("UPDATE receitas SET cliente_id=?, fonte_receita_id=?, descricao=?, valor=?, moeda=?, tipo=?, recorrencia=?, dia_vencimento=?, data_inicio=?, data_fim=?, nivel_esforco=?, equipe=?, score_roi=?, status=?, observacoes=? WHERE id=?");
        ...
        case 'despesas':
            if ($_SERVER['REQUEST_METHOD'] === 'GET') { respond($pdo->query("SELECT * FROM despesas WHERE ativo=1 ORDER BY descricao")->fetchAll()); }
            elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
                $d=getJsonInput(); $stmt=$pdo->prepare("INSERT INTO despesas (descricao, categoria_id, valor, moeda, recorrencia, dia_vencimento, observacoes, ativo) VALUES (?, ?, ?, ?, ?, ?, ?, 1)");
        ...
        case 'despesa':
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
                $d=getJsonInput(); $stmt=$pdo->prepare("UPDATE despesas SET descricao=?, categoria_id=?, valor=?, moeda=?, recorrencia=?, dia_vencimento=?, observacoes=?, ativo=? WHERE id=?");
                ...
                $pdo->commit(); respond(['message'=>'Despesa removida']);
            }
            break;

        case 'lancamentos':
            if ($_SERVER['REQUEST_METHOD'] === 'GET') {
                $mes_ano=$_GET['mes_ano']??date('Y-m'); $tipo=$_GET['tipo']??null; $status=$_GET['status']??null;
                ...
                $params=[$mes_ano]; if($tipo){$sql.=" AND l.tipo=?"; $params[]=$tipo;} if($status){$sql.=" AND l.status=?"; $params[]=$status;}
                $sql.=" ORDER BY l.data_vencimento"; $stmt=$pdo->prepare($sql); $stmt->execute($params); respond($stmt->fetchAll());
            } elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
                $d=getJsonInput(); $stmt=$pdo->prepare("INSERT INTO lancamentos (tipo, cliente_id, receita_id, despesa_id, descricao, valor, moeda, data_vencimento, data_pagamento, status, mes_referencia, observacoes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        case 'lancamento':
            $id=$_GET['id']??0; $scope=$_GET['scope']??'unico';
            if ($_SERVER['REQUEST_METHOD'] === 'GET') {
                $stmt=$pdo->prepare("SELECT l.*, c.razao_social as cliente_nome FROM lancamentos l LEFT JOIN clientes_cadastro c ON l.cliente_id=c.id WHERE l.id=?"); $stmt->execute([$id]);
                $lancamento=$stmt->fetch(PDO::FETCH_ASSOC); if(!$lancamento) respond(['error'=>'404'],false); respond($lancamento);
            } elseif ($_SERVER['REQUEST_METHOD'] === 'PUT') {
                $d=getJsonInput(); $stmt=$pdo->prepare("UPDATE lancamentos SET tipo=?, cliente_id=?, receita_id=?, despesa_id=?, descricao=?, valor=?, moeda=?, data_vencimento=?, data_pagamento=?, status=?, mes_referencia=?, observacoes=? WHERE id=?");
                ...
                $pdo->beginTransaction();
                $stmt=$pdo->prepare("SELECT receita_id, despesa_id FROM lancamentos WHERE id=?"); $stmt->execute([$id]); $item=$stmt->fetch(PDO::FETCH_ASSOC);
                if($scope==='todos' && $item){
                ...
                $pdo->commit(); respond(['message'=>'Removido']);
            }
            break;

        case 'marcar_pago':
            $id=$_GET['id']??0; $d=getJsonInput(); $pago=$d['pago']??true;
        ...
        case 'gerar_lancamentos':
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                $d=getJsonInput(); $mes_ano=$d['mes_ano']??date('Y-m'); $gerados=0; $ini=$mes_ano.'-01'; $fim=date('Y-m-t',strtotime($ini));
                ...
                foreach($receitas->fetchAll() as $r){
                    if($r['tipo']==='avulso'){
                        $checkAvulso=$pdo->prepare("SELECT id FROM lancamentos WHERE receita_id=?"); $checkAvulso->execute([$r['id']]); if($checkAvulso->fetch()) continue;
                    } else {
                        $int=$intervalos[$r['recorrencia']]??1;
                        if($int>1){ 
                            $l=$pdo->prepare("SELECT data_vencimento FROM lancamentos WHERE receita_id=? ORDER BY data_vencimento DESC LIMIT 1"); $l->execute([$r['id']]); $u=$l->fetch();
                            if($u){ $d1=new DateTime($u['data_vencimento']); $d1->modify('first day of this month'); $d2=new DateTime($ini); if((($d2->format('Y')-$d1->format('Y'))*12+($d2->format('m')-$d1->format('m')))<$int) continue; }
                        }
                    }
                    $check=$pdo->prepare("SELECT id FROM lancamentos WHERE receita_id=? AND mes_referencia=?"); $check->execute([$r['id'],$mes_ano]);
                    if(!$check->fetch()){
                        $dia=min($r['dia_vencimento']?:1, date('t',strtotime($ini))); $venc=$mes_ano.'-'.str_pad($dia,2,'0',STR_PAD_LEFT);
                        $pdo->prepare("INSERT INTO lancamentos (tipo,cliente_id,receita_id,descricao,valor,moeda,data_vencimento,status,mes_referencia) VALUES ('receita',?,?,?,?,?,?,?,?)")->execute([$r['cliente_id'],$r['id'],$r['cliente_nome']?:$r['descricao'],$r['valor'],$r['moeda']?:'BRL',$venc,'pendente',$mes_ano]); $gerados++;
                    }
                }
                // Despesas
                ...
                foreach($despesas->fetchAll() as $d){
                    ...
                    if($int>1){ $l=$pdo->prepare("SELECT data_vencimento FROM lancamentos WHERE despesa_id=? ORDER BY data_vencimento DESC LIMIT 1"); $l->execute([$d['id']]); $u=$l->fetch(); if($u){ $d1=new DateTime($u['data_vencimento']); $d1->modify('first day of this month'); $d2=new DateTime($ini); if((($d2->format('Y')-$d1->format('Y'))*12+($d2->format('m')-$d1->format('m')))<$int) continue; } }
                    $check=$pdo->prepare("SELECT id FROM lancamentos WHERE despesa_id=? AND mes_referencia=?"); $check->execute([$d['id'],$mes_ano]);
                    if(!$check->fetch()){
                        $dia=min($d['dia_vencimento']?:1, date('t',strtotime($ini))); $venc=$mes_ano.'-'.str_pad($dia,2,'0',STR_PAD_LEFT);
                        $pdo->prepare("INSERT INTO lancamentos (tipo,despesa_id,descricao,valor,moeda,data_vencimento,status,mes_referencia) VALUES ('despesa',?,?,?,?,?,?,?)")->execute([$d['id'],$d['descricao'],$d['valor'],$d['moeda']?:'BRL',$venc,'pendente',$mes_ano]); $gerados++;
                    }
                }
        ...
        case 'configuracoes': if($_SERVER['REQUEST_METHOD']==='GET'){ $s=$pdo->query("SELECT * FROM configuracoes"); $c=[]; foreach($s->fetchAll() as $r) $c[$r['chave']]=$r['valor']; respond($c); } elseif($_SERVER['REQUEST_METHOD']==='PUT'){ $d=getJsonInput(); foreach($d as $k=>$v){ $pdo->prepare("UPDATE configuracoes SET valor=? WHERE chave=?")->execute([$v,$k]); } respond(['message'=>'Salvo']); } break;
        case 'upload_anexo': 
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                $f = $_FILES['arquivo']; $ext = strtolower(pathinfo($f['name'], PATHINFO_EXTENSION)); $n = uniqid().'_'.preg_replace('/[^a-zA-Z0-9\._-]/','',$f['name']);
        ...
        case 'delete_anexo':
            if($_SERVER['REQUEST_METHOD']==='DELETE'){
                $s=$pdo->prepare("SELECT caminho FROM clientes_anexos WHERE id=?"); $s->execute([$_GET['id']]); $a=$s->fetch();
                if($a && file_exists(__DIR__.'/../'.$a['caminho'])) unlink(__DIR__.'/../'.$a['caminho']);
                $pdo->prepare("DELETE FROM clientes_anexos WHERE id=?")->execute([$_GET['id']]); respond(['message'=>'Anexo removido']);
            }
            break;
            
        default: http_response_code(400); respond(['error' => 'Ação não reconhecida'], false);
    }
...
function respond($data, $success = true) { echo json_encode(['success' => $success, 'data' => $data], JSON_UNESCAPED_UNICODE); exit; }
function getJsonInput() { $json = file_get_contents('php://input'); return json_decode($json, true) ?? []; }
function obterCotacaoDolar($pdo) {
    $stmt = $pdo->prepare("SELECT valor, updated_at FROM configuracoes WHERE chave = 'cotacao_dolar'"); $stmt->execute(); $config = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    try {
        $curl = curl_init(); curl_setopt_array($curl, [CURLOPT_URL => "https://economia.awesomeapi.com.br/last/USD-BRL", CURLOPT_RETURNTRANSFER => true, CURLOPT_TIMEOUT => 3, CURLOPT_SSL_VERIFYPEER => false]);
